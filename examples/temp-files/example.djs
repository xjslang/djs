...
(async function main() {
  ...
  let tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'djs-example-')) or |err| {
    console.error('Failed to create temp directory:', err.message)
    return
  }
  defer fs.rm(tmpDir, { recursive: true })
  ...
  await fs.writeFile(file1, JSON.stringify(data1, null, 2)) or |err| {
  ...
  await fs.writeFile(file2, 'This is a temporary text file\nLine 2\nLine 3') or |err| {
  ...
  let content = await fs.readFile(file1, 'utf8') or |err| {
  ...
  await fs.writeFile(file3, logEntry) or |err| {
  ...
  let files = await fs.readdir(tmpDir) or |err| {
